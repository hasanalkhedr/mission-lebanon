...
@section('content')
    ...
    <form action="{{ route('mission_orders.update', $missionOrder->id) }}" method="POST" class="w-full">
        ...
        <div class="flex flex-wrap -mx-3 mb-2" id="advance_amount_container" style="display: none;">
            <div class="w-1/2 px-3">
                <x-label>
                    Montant de l'avance ({{$missionOrder->bareme->currency}})<span class="text-red-500">*</span>
                </x-label>
                <x-text-input name="advance" value="{{  $missionOrder->advance ?? 0 }}" id="advance_amount_input" />
